...
export class BuilderService {
  ...
  async analyze(rawInput: string): Promise<{ analysis: PromptAnalysis; inputTokens: number; outputTokens: number }> {
  ...
  async generate(
  ...
  ): Promise<{ builtPrompt: BuiltPrompt; inputTokens: number; outputTokens: number }> {
    const claude = createClaudeClient(this.env);
    ...
    try {
      const response = await claude.messages.create(params);
      const genBlock = response.content.find((b) => b.type === "text");
      const text = genBlock?.type === "text" ? genBlock.text : "";
      const builtPrompt = JSON.parse(text) as BuiltPrompt;
