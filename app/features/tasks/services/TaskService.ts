...
import type { DecomposeRequest, ReviewVerdict } from "~/features/tasks/schemas/tasks.schema";
...
export class TaskService {
  ...
  private readonly logRepo: TaskLogRepository;
  ...
  async decompose(req: DecomposeRequest): Promise<{ task: Task; subTasks: SubTask[] }> {
    ...
    try {
      ...
      const items = JSON.parse(text) as Array<{ title: string; description: string }>;
  ...
  async executePhase(
    ...
    const model = (task?.model ?? "claude-opus-4-8") as ModelId;

    const [systemPrompt, userContent, callModel] = this.buildPhasePrompt(
      phase,
      subTask,
      context,
      model,
    ...
    try {
      const response = await claude.messages.create(params);
      const phaseBlock = response.content.find((b) => b.type === "text");
      const output = phaseBlock?.type === "text" ? phaseBlock.text : "";
