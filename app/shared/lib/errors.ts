...
export class RateLimitError extends AppError {
  ...
  toResponse(): Response {
    return Response.json(
      { error: this.code, message: this.message, retryAfter: this.retryAfter },
...
export function toAppError(err: unknown): AppError {
  ...
  if (err && typeof err === "object" && "status" in err) {
    ...
    if (status === 400) return new ClaudeParamError(message);
    if (status === 401) return new ClaudeAuthError();
    if (status === 429) return new ClaudeRateLimitError();
    if (status === 529 || status === 500) return new ClaudeOverloadError();
  }
