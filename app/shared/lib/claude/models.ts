...
import type { ModelConfig, ModelId, EffortLevel, CoreMessage, BuildMessageParamsOptions } from "~/shared/types/claude";
...
export function buildMessageParams(
  model: ModelConfig,
  opts: BuildMessageParamsOptions,
  ...
  if (model.supportsEffort && opts.effort) {
    const effort: EffortLevel =
      opts.effort === "xhigh" && !model.supportsXHighEffort ? "high" : opts.effort;
  ...
  if (model.supportsThinking && opts.thinking) {
    (params as unknown as Record<string, unknown>)["thinking"] = { type: "enabled", budget_tokens: 10000 };
  }
